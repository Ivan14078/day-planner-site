...
const API_URL = "https://day-planner-bot-production.up.railway.app";
...
export default function DayPlanner() {
  ...
  useEffect(() => {
    (async () => {
      try {
        if (getInitData()) {
          const remote = await apiRequest("/api/tasks");
          setTasks(remote.map(t => ({
            id: t.id, title: t.title, date: t.date, time: t.time || "", endTime: t.end_time || "",
            category: t.category || "work", done: !!t.done, reminderMinutes: t.reminder_minutes || 0,
          })));
          ...
          const raw = localStorage.getItem("day-planner-tasks");
  ...
  useEffect(() => {
    if (!loaded) return;
    if (getInitData()) return; // backend handles persistence via per-action calls
    try {
  ...
  const dayTasks = useMemo(
    () => tasks
      ...
      .filter(t => {
        if (selected !== todayKey()) return true;
        const finish = t.endTime || t.time;
        if (!finish) return true;
        return finish >= nowHHMM;
      })
      .sort((a, b) => (a.time || "99:99").localeCompare(b.time || "99:99")),
    [tasks, selected, nowHHMM]
  );
  ...
  function addTask() {
    ...
    setError("");
    const newTask = { id: String(Date.now()), title: title.trim(), time, endTime, date: selected, done: false, category, reminderMinutes: Number(reminderMinutes) };
    setTasks(ts => [...ts, newTask]);
    setTitle(""); setTime(""); setEndTime(""); setReminderMinutes(0);
    if (getInitData()) {
      apiRequest("/api/tasks", { method: "POST", body: JSON.stringify(newTask) })
  ...
  function toggleDone(id) {
    setTasks(ts => {
      ...
      if (getInitData()) {
        ...
        apiRequest(`/api/tasks/${id}`, { method: "PATCH", body: JSON.stringify({ done: task.done }) }).catch(() => {});
      }
      return updated;
    });
  }
  ...
  function deleteTask(id) {
    ...
    if (getInitData()) {
      apiRequest(`/api/tasks/${id}`, { method: "DELETE" }).catch(() => {});
    }
  }

  function reschedule(id, newDate) {
    setTasks(ts => ts.map(t => t.id === id ? { ...t, date: newDate } : t));
    setMovingId(null);
    if (getInitData()) {
      apiRequest(`/api/tasks/${id}`, { method: "PATCH", body: JSON.stringify({ date: newDate }) }).catch(() => {});
    }
  }
  ...
  return (
    <div style={{ fontFamily: "'Work Sans', sans-serif", background: COLORS.bg, color: COLORS.ink, padding: "24px", borderRadius: "16px", maxWidth: "760px" }}>
      ...
      {syncStatus && (
        <div style={{
          fontSize: "12px", marginBottom: "12px", padding: "6px 10px", borderRadius: "8px",
          background: syncStatus === "ok" ? COLORS.greenTint : COLORS.orangeTint,
          color: syncStatus === "ok" ? COLORS.greenDeep : COLORS.danger,
        }}>
          {syncStatus === "ok" ? "✓ Синхронизировано с сервером" : "⚠ Не удалось сохранить на сервере — напоминание может не прийти"}
        </div>
      )}
